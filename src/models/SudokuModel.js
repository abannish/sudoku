...
function CRME(sudokuBoard) {
    /*
        scan each cell in the grid from left to right, top to bottom
            for each cell:
                set possible values to 123456789
                scan its column and eliminate the values already present in the column
                scan its minigride and eliminate the values already present in the minigrid
                if there is only one possible value for the cell, then the number is known
            contiue for all cells
    */

    for(let row = 0; row < sudokuBoard.length; row++) {
        for(let column = 0; column < sudokuBoard[row].length; column++) {
            let possibilities = [1,2,3,4,5,6,7,8,9];
            findColumnPossibilities(sudokuBoard,column,possibilities);
            findRowPossibilities(sudokuBoard,row,possibilities);
            findMinigridPossibilities(sudokuBoard,row,column,possibilities);
        }
    }
}
